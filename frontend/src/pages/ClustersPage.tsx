...
export default function ClustersPage() {
  ...
  const getSeverityPillClass = (severity: string) => {
    switch (severity.toLowerCase()) {
      case 'critical': return styles.pillCritical
      case 'high': return styles.pillHigh
      case 'medium': return styles.pillMedium
      default: return styles.pillLow
    }
  }
            ...
            <tbody>
              {loading ? (
                ...
                clusters.map(cluster => (
                  <tr key={cluster.id}>
                    <td data-label="Title">{cluster.title}</td>
                    <td data-label="Type" className={styles.cellMono}>{cluster.type}</td>
                    <td data-label="Severity">
                      <span className={`${styles.pill} ${getSeverityPillClass(cluster.severity)}`}>
                        {cluster.severity}
                      </span>
                    </td>
                    <td data-label="Count" className={styles.cellNowrap}>{cluster.eventCount}</td>
                    <td data-label="Last Seen" className={styles.cellNowrap}>{formatDate(cluster.lastSeen)}</td>
                  </tr>
                ))
              )}
            </tbody>
